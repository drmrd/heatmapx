...
@pytest.mark.parametrize(
    'graph_class', [nx.Graph, nx.DiGraph, nx.MultiDiGraph, nx.OrderedDiGraph]
)
...
@pytest.mark.parametrize(
    'node_labels', [[1, 3, 5, 7], ['n1', 'n2', 'n3', 'n4']]
)
...
def test_components_disconnected_from_sources_are_assigned_coldest_temperature(
    graph_class,
):
    graph: nx.Graph = graph_class([(0, 1), (1, 2), (3, 4)])

    temperature_graph = hx.temperature_graph(
        graph, [0], increments=range(1, 4)
    )

    source_connected_component_max_temperature = max(
        *(
            heat
            for node, heat in temperature_graph.nodes(data='heat')
            if node <= 2
        ),
        *(
            heat
            for *_, heat in temperature_graph.edges({0, 1, 2}, data='heat')
        ),
    )
...
def test_heat_distribution_respects_edge_directedness():
    ...
    temperature_graph = hx.temperature_graph(
        square_graph, [0], increments=range(3)
    )
...
def test_accepts_iterable_of_depth_specific_heat_increments(graph_class):
    ...
    hx.temperature_graph(square_graph, sources=[0], increments=[1, 0.5, 0.25])
...
def test_the_last_increment_in_an_iterable_is_repeated_indefinitely():
    ...
    temperature_graph = hx.temperature_graph(
        long_path, sources=[0], increments=iter(increments)
    )
...
def test_providing_an_empty_increments_iterable_results_in_an_informative_error():  # noqa: E501
    with pytest.raises(
        ...
        hx.temperature_graph(
            nx.Graph([(0, 1)]), sources=[0], increments=iter(())
        )
...
def test_can_limit_distance_heat_spreads_from_heat_sources(graph_class):
    ...
    heat_source = 0
    temperature_graph = hx.temperature_graph(
        square_graph, sources=[heat_source], max_depth=1
    )
