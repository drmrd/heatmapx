...
class TestHeatGraphWithIncrements:
    ...
    def test_components_disconnected_from_sources_are_assigned_coldest_heat(
            self, graph_class
    ):
        ...
        source_connected_component_max_heat = max(
            *(
                heat for node, heat in heat_graph.nodes(data='heat')
                if node <= 2
            ),
            *(
    ...
    def test_the_last_increment_in_an_iterable_is_repeated_indefinitely(self):
    ...
    def test_passing_empty_increments_iterable_results_in_an_informative_error(
            self
    ):
        with pytest.raises(
    ...
    def test_unreachable_nodes_and_edges_are_assigned_minimum_heat_value(self):
