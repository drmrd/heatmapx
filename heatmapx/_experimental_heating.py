...
def heat_graph_with_increments(
    ...
    max_depth : int, optional
        ...
        updating node and edge heat values. (Equivalently, the maximum
        graph distance away from a source node at which to update
        heat values.) If left unspecified, all nodes and edges reachable
    ...
    for source in sources:
        ...
        for edges_at_depth, (increment, next_increment) in data_by_depth:
            for edge in edges_at_depth:
                _update_edge_heat(T, edge, key, increment,
                                  next_increment, weight)
                _update_incident_node_heat_values(T, edge[:2], key, increment,
                                                  next_increment, weight,
                                                  visited_nodes)
    _update_heat_values_of_unreachable_nodes_and_edges(T, sources, key)
    return T
...
def _update_edge_heat(G, edge, key, increment, next_increment,
                      weight):
    G.edges[edge][key] += G.edges[edge].get(weight, 1) * increment


def _update_incident_node_heat_values(G, nodes, key, increment,
...
def _update_heat_values_of_unreachable_nodes_and_edges(T, sources, key):
    unreachable_nodes = set(T) - set(sources) - set().union(*(
        nx.descendants(T, source) for source in sources
    ))
    coldest_heat_value = max(
    ...
    for unreachable_node in unreachable_nodes:
        T.nodes[unreachable_node][key] = coldest_heat_value
    ...
    for edge in T.edges(unreachable_nodes, **edges_iterator_kwargs):
        T.edges[edge][key] = coldest_heat_value
